...
test("main.js: saqlash kaliti, sarlavha va bosqich nomlari", () => {
  assert.equal(captured.storageKey, "xotira-ombori:v1");
  assert.equal(captured.title, "Xotira ombori");
  assert.deepEqual(captured.stageTitles, ["Oʻlchov zinapoyasi", "Kattasini top", "Nechta sigʻadi?"]);
});
