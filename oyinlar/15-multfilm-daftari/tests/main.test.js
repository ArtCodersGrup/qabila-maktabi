...
test("main.js: saqlash kaliti, sarlavha va bosqich nomlari", () => {
  assert.equal(captured.storageKey, "multfilm-daftari:v1");
  assert.equal(captured.title, "Multfilm daftari");
  assert.deepEqual(captured.stageTitles, ["Kadrlar", "Video hajmi", "Faqat oʻzgargani"]);
});
