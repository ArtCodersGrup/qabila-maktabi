...
test("main.js: saqlash kaliti, sarlavha va bosqich nomlari", () => {
  assert.equal(captured.storageKey, "ikkilik-hisobchi:v1");
  assert.equal(captured.title, "Ikkilik hisobchi");
  assert.deepEqual(captured.stageTitles, ["Qoʻshish", "Ayirish", "Koʻpaytirish"]);
});
