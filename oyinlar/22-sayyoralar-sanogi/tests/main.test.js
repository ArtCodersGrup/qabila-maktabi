...
test("main.js: saqlash kaliti, sarlavha va bosqich nomlari", () => {
  assert.equal(captured.storageKey, "sayyoralar-sanogi:v1");
  assert.equal(captured.title, "Sayyoralar sanogʻi");
  assert.deepEqual(captured.stageTitles, ["Qoʻshish", "Ayirish", "Koʻpaytirish va jumboq"]);
});
